...
page = Blueprint('view4', __name__)
client = MongoClient('localhost', 27017)
db = client['diva-proj']
mycol1=db['author_jason']
...
def author_jason():
    author_works = list(mycol1.find({'Jason Priem': {'$exists': True}}))[0]
    ...
    for p in all_referenced:
        if p['cited_by_count'] is not None: 
            if p['cited_by_count']> 5 and len([c['display_name'] for c in p['concepts'] if c['level'] == 0])>0:
                filtered.append(p)
...
@page.route("/view4")
def view4_index():
    return redirect(url_for('static', filename='view4/index.html'))


@page.route("/view4/authorsjason")
def get_author_data1():
    data = author_jason()
    #print(data)
    return jsonify(data)