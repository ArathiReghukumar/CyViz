...
page = Blueprint('view11', __name__)
client = MongoClient('localhost', 27017)
db = client['diva-proj']
mycol8=db['authors_zhang']
...
def authors_zhang():
    author_works = list(mycol8.find())
    author_works = author_works[0]['Desheng Zhang']
    g2=nx.Graph()
    author_name='Desheng Zhang'
...
@page.route("/view11")
def view11_index():
    return redirect(url_for('static', filename='view11/index.html'))


@page.route("/view11/authorszhang")
def get_author_data8():
    data = authors_zhang()
    #print(data)
    return jsonify(data)